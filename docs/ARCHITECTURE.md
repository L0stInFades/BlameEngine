...
| **植被系统(核心切片)** `next_vegetation` | **prototype(core-only)** | 仅 headless 核心:数据驱动 `VegetationDef` + `VegetationBuilder` + fail-closed `VegetationValidator` + **确定性 per-cell scatter**(抖动网格 + 最小间距;种子来自 `masterSeed`+世界 cell 坐标 → 顺序无关、可复现)+ `CellLayer::Vegetation` POD blob pack/unpack + `QueryRadiusXZ`;18 单测 + ASan([ADR-0014](adr/0014-vegetation-system.md))。**越界判定已定**(核心拥有放置/逐株状态、UE5 只渲染)。**但交付链路未通**:`CellLayer::Vegetation` 仍 placeholder(`src/streaming_manager.cpp:1069`)、无 layered `.ncell`/cook 工具/runtime store/UE5 消费端;wire 契约待定(logicalRadius 未入 blob、instanceId 仅 32-bit hash)。**交付门见 ADR-0014**,在其全过前不得标 usable |
